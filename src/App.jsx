    ...
    <>
      ...
      <h1 className='font-bold text-center text-2xl'>iTask - Manage your todos at one place</h1>
        <div className="addTodo my-5 flex flex-col gap-4">
          ...
          <button onClick={handleAdd} disabled={todo.length<=3} className="bg-violet-800 disabled:bg-violet-700 mx-2 hover:bg-violet-950 p-4 py-2 text-sm font-bold text-white rounded-full">
            Save
          </button>
          </div>
        </div>
        <input className="my-4" onClick={toggleFinished} type="checkbox" checked={showFinished} />
        ...
        <div className="todos">
          ...
          return (showFinished || !item.isCompleted) && <div key={item.id} className="todo flex my-3 justify-between">
            ...
            <input name={item.id} onChange={hadleCheckbox} type="checkbox" checked={item.isCompleted} id="" />
